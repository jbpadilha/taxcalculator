...
import * as genericServer from '../../server/GenericServer';
...
describe('Main component', () => {
  ...
  test('calculates taxes correctly', async () => {
    ...
    // Mock API response
    jest.spyOn(genericServer, 'fetchTaxRates').mockResolvedValue(
        {
            "tax_brackets": [
                {
                    "min": 0,
    ...
    /*jest.mock(fetchTaxRates.prototype, () => {
        jest.fn(
            () => Promise.resolve(
                {
                    "tax_brackets": [
    ...
    expect(fetchTaxRates).toHaveBeenCalled();
